...
function generateScript(
  ...
  const oi = variation % openingPool.length;
  const bi = variation % bridgePool.length;

  return {
    scenario,
    opening: openingPool[oi],
    bridge: bridgePool[bi],
  };
}
...
const Field = ({
  ...
  <div>
    ...
    <select
      ...
      className="w-full px-4 py-2.5 rounded-lg border border-border bg-card text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-primary/50 appearance-none"
      style={{
...
const CopyBtn = ({ text }: { text: string }) => {
  ...
  return (
    <button
      onClick={copy}
      className="absolute top-3 right-3 p-1.5 rounded-md bg-card/80 border border-border/50 text-muted-foreground hover:text-foreground transition-colors"
    ...
    <div ref={topRef}>
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <p className="text-muted-foreground text-center mb-8 text-sm">
          Tell us about the call and we'll craft a conversation flow tailored to your scenario.
        </p>

        {/* ─── Form ─── */}
        <div className="rounded-xl border border-border/50 bg-card/50 backdrop-blur-sm p-6 mb-8">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Field label="What type of call is this?" value={callType} onChange={setCallType} options={callTypes} />
            <Field label="What industry is the prospect in?" value={industry} onChange={setIndustry} options={industries} />
            <Field label="What's the prospect's role?" value={role} onChange={setRole} options={roles} />
            <Field label="How many employees?" value={size} onChange={setSize} options={sizes} />
            <div className="sm:col-span-2">
              <Field label="Main service to discuss?" value={service} onChange={setService} options={services} />
            </div>
          </div>
          ...
          <button
            ...
            className="mt-6 w-full py-2.5 rounded-lg bg-primary text-primary-foreground text-sm font-semibold hover:shadow-[0_0_20px_hsl(var(--primary)/0.4)] transition-all duration-300 disabled:opacity-40 disabled:cursor-not-allowed"
          >
        ...
        <AnimatePresence>
          {generating && !script && (
            ...
            >
              ...
              />
              <p className="text-muted-foreground">Crafting your conversation flow…</p>
            </motion.div>
          )}
        </AnimatePresence>
        ...
        {script && !generating && (
          ...
          >
            ...
            <div className="flex items-center justify-between mb-6">
              <p className="text-xs font-mono uppercase tracking-wider text-muted-foreground">{script.scenario}</p>
              <button
            ...
            {/* Opening */}
            <div className="relative mb-4 rounded-xl border-l-4 border-l-primary border border-border/50 bg-card/50 backdrop-blur-sm p-5">
              ...
              <p className="text-sm text-foreground leading-relaxed pr-8">{script.opening}</p>
            </div>

            {/* Bridge */}
            <div className="relative mb-6 rounded-xl border-l-4 border-l-[hsl(280,60%,55%)] border border-border/50 bg-card/50 backdrop-blur-sm p-5">
              ...
              <p className="text-sm text-foreground leading-relaxed pr-8">{script.bridge}</p>
            </div>
            ...
            <div className="relative rounded-xl border border-border/30 bg-card/30 p-6 mb-6 select-none pointer-events-none" aria-hidden>
            ...
            {!submitted ? (
              <div className="rounded-xl border border-primary/20 bg-card/60 backdrop-blur-sm p-6 text-center">
                <h3 className="text-xl font-bold text-foreground mb-2">Unlock the Complete Script</h3>
                <p className="text-sm text-muted-foreground mb-5">
                  Get the full conversation flow including discovery questions, objection handling, value statements, and close.
                </p>
                ...
                <div className="text-left max-w-sm mx-auto mb-6">
                  ...
                  <ul className="space-y-1.5">
                    {[
                      "Discovery Questions (5-7 tailored questions)",
                      "Industry-Specific Risk Talking Points",
                      "Objection Responses",
                      "Value Statement",
                      "Suggested Next Steps & Close",
                    ].map((item) => (
                ...
                <form onSubmit={handleGateSubmit} className="max-w-sm mx-auto space-y-3">
                  ...
                  <p className="text-xs text-muted-foreground/50">
                    No spam. Just your script and occasional growth tips.
                  </p>
                </form>
              </div>
            ) : (
              ...
              >
                ...
                <p className="text-sm text-muted-foreground">
                  Your complete script is on its way to <span className="text-foreground font-medium">{email}</span>.
                </p>
              </motion.div>
            )}
          </motion.div>
